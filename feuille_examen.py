...
roi_dict = {
    ...
    "note_exament": [0.7933,0.1368,0.1657,0.0984]
}
...
def detect_table_contours(table_image):
    ...
    for every colunm in the second half show the cell, then apply the mathematical morphology : dilation to know the marked cell.
    ...
    for i, (x, y, w, h) in enumerate(cell_contours):
        if i >= (len(cell_contours)//2)+1:
            cell = table_image[y:y+h, x:x+w]
            # Apply dilation to detect marked cells
            dilated_cell = cv2.dilate(cell, np.ones((3, 3), np.uint8), iterations=5)
            ...
            if non_zero_count > max_non_zero_count:
                ...
                if len(cell_contours)>20:
                    marked_exam_grade = max_cell_index - (len(cell_contours)//2)+1
                #extract the decimal Number of the exam grade : 00, 25, 50, 75
                else:
                    decimal_grade_index =( max_cell_index + 1) // 4 

    # Show only the cell with the biggest non-zero count
    if max_cell_index != -1:
...
if __name__ == "__main__":
    image_path = r"examens\anis121125_page-0001.jpg"  # Replace with your image path
    ...
    detect_table_contours(table_image)
    table_image2 = extract_table(image, roi_dict["table_2"])
    detect_table_contours(table_image2)
